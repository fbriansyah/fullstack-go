...
// SimpleAuthService is a simplified version without database transactions for testing
...
// NewSimpleAuthService creates a new simple auth service instance
...
func (s *SimpleAuthService) Login(ctx context.Context, cmd *LoginCommand) (*AuthResult, error) {
	...
	if err != nil {
		return nil, NewInternalError(fmt.Sprintf("failed to create session: %v", err))
	}

	// Save session
	if err := s.sessionRepo.Create(ctx, session); err != nil {
		return nil, NewInternalError(fmt.Sprintf("failed to save session: %v", err))
	}

	// Publish user logged in event
	event := domain.NewUserLoggedInEvent(user.ID, session.ID, cmd.IPAddress, cmd.UserAgent)
	if err := s.eventBus.Publish(ctx, event); err != nil {
		return nil, NewInternalError(fmt.Sprintf("failed to publish login event: %v", err))
	}
...
func (s *SimpleAuthService) Register(ctx context.Context, cmd *RegisterCommand) (*AuthResult, error) {
	if err := cmd.Validate(); err != nil {
		return nil, err
	}
	...
	user, err := s.userService.CreateUser(ctx, createUserCmd)
	if err != nil {
		...
		return nil, NewInternalError(fmt.Sprintf("failed to create user: %v", err))
	}
	...
	if err := s.eventBus.Publish(ctx, event); err != nil {
		return nil, NewInternalError(fmt.Sprintf("failed to publish registration event: %v", err))
	}
...
func (s *SimpleAuthService) Logout(ctx context.Context, cmd *LogoutCommand) error {
	if err := cmd.Validate(); err != nil {
		return err
	}
	...
	// Delete session
	if err := s.sessionRepo.Delete(ctx, cmd.SessionID); err != nil {
...
func (s *SimpleAuthService) ValidateSession(ctx context.Context, query *ValidateSessionQuery) (*SessionValidationResult, error) {
...
func (s *SimpleAuthService) RefreshSession(ctx context.Context, cmd *RefreshSessionCommand) (*domain.Session, error) {
	if err := cmd.Validate(); err != nil {
		return nil, err
	}
	...
	session, err := s.sessionRepo.GetByID(ctx, cmd.SessionID)
	if err != nil {
		return nil, NewSessionNotFoundError(cmd.SessionID)
	}
	...
	if cmd.IPAddress != "" || cmd.UserAgent != "" {
		if !session.ValidateSecurityContext(cmd.IPAddress, cmd.UserAgent) {
			return nil, NewSessionInvalidError()
		}
	}
	...
	if err := s.sessionRepo.Update(ctx, session); err != nil {
		return nil, NewInternalError(fmt.Sprintf("failed to update session: %v", err))
	}
...
func (s *SimpleAuthService) ChangePassword(ctx context.Context, cmd *ChangePasswordCommand) error {
	if err := cmd.Validate(); err != nil {
		return err
	}
	...
	_, err = s.userService.ChangeUserPassword(ctx, changePasswordCmd)
	if err != nil {
	...
	if err := s.sessionRepo.DeleteByUserID(ctx, cmd.UserID); err != nil {
...
func (s *SimpleAuthService) CleanupExpiredSessions(ctx context.Context) error {
